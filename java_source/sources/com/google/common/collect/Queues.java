...
public final class Queues {
    ...
    @GwtIncompatible
    public static <E> ArrayBlockingQueue<E> newArrayBlockingQueue(int i10) {
    ...
    @GwtIncompatible
    public static <E> LinkedBlockingDeque<E> newLinkedBlockingDeque(int i10) {
        return new LinkedBlockingDeque<>(i10);
    }

    @GwtIncompatible
    public static <E> LinkedBlockingQueue<E> newLinkedBlockingQueue(int i10) {
    ...
    public static <E> Deque<E> synchronizedDeque(Deque<E> deque) {
        return (Deque<E>) new Synchronized.SynchronizedObject(deque, null);
    }

    public static <E> Queue<E> synchronizedQueue(Queue<E> queue) {
        if (!(queue instanceof Synchronized.SynchronizedQueue)) {
            return (Queue<E>) new Synchronized.SynchronizedObject(queue, null);
        }
        return queue;
    }
    ...
    public static <E> int drain(BlockingQueue<E> blockingQueue, Collection<? super E> collection, int i10, long j10, TimeUnit timeUnit) throws InterruptedException {
    ...
    @Beta
    @GwtIncompatible
    public static <E> int drainUninterruptibly(BlockingQueue<E> blockingQueue, Collection<? super E> collection, int i10, long j10, TimeUnit timeUnit) {
        E poll;
        Preconditions.checkNotNull(collection);
        long nanos = timeUnit.toNanos(j10) + System.nanoTime();
        int i11 = 0;
