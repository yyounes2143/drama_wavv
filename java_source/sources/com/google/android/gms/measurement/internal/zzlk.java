package com.google.android.gms.measurement.internal;

import android.os.Bundle;
import java.util.List;
import java.util.Map;

/* compiled from: com.google.android.gms:play-services-measurement-base@@23.0.0 */
/* loaded from: classes9.dex */
public interface zzlk {
    void zza(String str, String str2, Bundle bundle);

    void zzb(String str, String str2, Bundle bundle, long j10);

    Map zzd(String str, String str2, boolean z10);

    void zze(zzjp zzjpVar);

    void zzf(zzjq zzjqVar);

    void zzg(zzjq zzjqVar);

    String zzh();

    String zzi();

    String zzj();

    String zzk();

    long zzl();

    void zzm(String str);

    void zzn(String str);

    void zzo(Bundle bundle);

    void zzp(String str, String str2, Bundle bundle);

    List zzq(String str, String str2);

    int zzr(String str);

    Object zzx(int i10);
}
