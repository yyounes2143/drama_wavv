...
public class GoogleApiManager implements Handler.Callback {
    ...
    public static Status zaF(ApiKey apiKey, ConnectionResult connectionResult) {
        return new Status(connectionResult, C4305v.m11590a("API: ", apiKey.zaa(), " is not available on this device. Connection failed with: ", String.valueOf(connectionResult)));
    }
            ...
            case 5:
                ...
                if (zabqVar != null) {
                    if (connectionResult.getErrorCode() == 13) {
                        zabq.zai(zabqVar, new Status(17, C4305v.m11590a("Error resolution was canceled by the user, original error message: ", this.zaj.getErrorString(connectionResult.getErrorCode()), ": ", connectionResult.getErrorMessage())));
                    } else {
                        zabq.zai(zabqVar, zaF(zabq.zag(zabqVar), connectionResult));
                    }
                } else {
                    Log.wtf("GoogleApiManager", C3477d.m6716a(i10, "Could not find API instance ", " while trying to fail enqueued calls."), new Exception());
                }
                return true;
            case 6:
