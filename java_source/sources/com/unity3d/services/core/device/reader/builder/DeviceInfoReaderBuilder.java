...
public class DeviceInfoReaderBuilder {
    ...
    private JsonFlattenerRules getTsiRequestStorageRules() {
        return new JsonFlattenerRules(Arrays.asList("privacy", "gdpr", "framework", "adapter", "unity", "pipl", C23966i.f109566c, "user", JsonStorageKeyNames.UNIFIED_CONFIG_KEY), Collections.singletonList("value"), Arrays.asList(MaxEvent.f109003b, "exclude", "pii", "nonBehavioral", "nonbehavioral"));
    }
    ...
    public JsonFlattenerRules getMediationInfoRules() {
        return new JsonFlattenerRules(Collections.singletonList("mediation"), Collections.singletonList("value"), Arrays.asList(MaxEvent.f109003b, "traits"));
    }
