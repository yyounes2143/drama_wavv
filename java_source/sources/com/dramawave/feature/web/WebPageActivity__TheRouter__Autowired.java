package com.dramawave.feature.web;

import androidx.annotation.Keep;
import com.dramawave.core.router.path.WebPage;
import com.dramawave.core.router.path.WebPageArgs;
import java.util.Iterator;
import java.util.LinkedList;
import p704p8.C28196d;
import p748t8.C28560a;
import p794x8.InterfaceC28822a;

@Keep
/* loaded from: classes2.dex */
public class WebPageActivity__TheRouter__Autowired {
    public static final String TAG = "Created by kymjs, and APT Version is 1.2.3.";
    public static final String THEROUTER_APT_VERSION = "1.2.3";

    public static void autowiredInject(Object obj) {
        if (obj instanceof WebPageActivity) {
            WebPageActivity webPageActivity = (WebPageActivity) obj;
            Iterator<InterfaceC28822a> it = C28196d.f123333a.iterator();
            while (it.hasNext()) {
                try {
                    WebPageArgs webPageArgs = (WebPageArgs) it.next().mo53774a("com.dramawave.core.router.path.WebPageArgs", webPageActivity, new C28560a("com.dramawave.core.router.path.WebPageArgs", WebPage.PARAMS_WEB_PAGE_ARGS, "com.dramawave.feature.web.WebPageActivity", WebPage.PARAMS_WEB_PAGE_ARGS));
                    if (webPageArgs != null) {
                        webPageActivity.webPageArgs = webPageArgs;
                    }
                } catch (Exception unused) {
                    LinkedList<InterfaceC28822a> linkedList = C28196d.f123333a;
                }
            }
        }
    }
}
