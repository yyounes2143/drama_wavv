...
public class NetworkBinder {
            ...
            synchronized (NetworkBinder.this.mLocker) {
                try {
                    if (NetworkBinder.this.mIsActivatingNetwork && NetworkBinder.this.checkNativeValid()) {
                        ...
                        LiteavLog.m46693e(NetworkBinder.TAG, "[BindSocket]:activate network timeout");
    ...
    public static native void nativeOnNetworkAvailable(long j10, String str, boolean z10);

    public boolean activateNetworkAdapter(int i10) {
        if (i10 != 1 && i10 != 2) {
            return false;
        }
        try {
        } catch (Exception unused) {
            LiteavLog.m46693e(TAG, "[BindSocket]:activateNetworkAdapter in java encountered an exception, networkType:".concat(String.valueOf(i10)));
        }
        synchronized (this.mLocker) {
            try {
                if (this.mIsActivatingNetwork) {
                    LiteavLog.m46693e(TAG, "[BindSocket]:mIsActivatingNetwork is true");
                    return false;
                }
                this.mIsActivatingNetwork = true;
                this.mIsNetworkAvailable = false;
                if (this.mNetworkType != i10) {
                    deactivateNetworkAdapter();
                }
                this.mNetworkType = i10;
                if (doActivateNetworkAdapter(i10)) {
                    synchronized (this.mLocker) {
                        try {
    ...
    public boolean doActivateNetworkAdapter(int i10) {
        int i11;
        ...
        try {
            ...
            connectivityManager.requestNetwork(new NetworkRequest.Builder().addCapability(12).addTransportType(i11).build(), createNetworkCallback);
            return true;
        } catch (Exception unused) {
            LiteavLog.m46693e(TAG, "[BindSocket]:requestNetwork in java encountered an exception, networkType:".concat(String.valueOf(i10)));
        }
    }
    ...
    public static boolean bindSocket(String str, int i10) {
        if (str != null && !str.isEmpty() && i10 != -1) {
            try {
                ...
                if (findNetwork == null) {
                    LiteavLog.m46693e(TAG, "[BindSocket]:NetworkBinder bindSocket can not find network:".concat(str));
                    return false;
                }
                findNetwork.bindSocket(ParcelFileDescriptor.fromFd(i10).getFileDescriptor());
                LiteavLog.m46697i(TAG, "[BindSocket]:NetworkBinder bindSocket succ, name:" + str + " socket:" + i10);
                return true;
            } catch (Exception unused) {
                LiteavLog.m46693e(TAG, "[BindSocket]:bindSocket in java encountered an exception, name:" + str + " socket:" + i10);
            }
        }
        return false;
    }
    ...
    public void cancleTimeoutRunnable() {
        if (this.mHandler != null) {
            Log.m46645i(TAG, "[BindSocket]:remove cancel last request network runnable", new Object[0]);
            ...
            try {
                ...
                ConnectivityManager.NetworkCallback networkCallback2 = new ConnectivityManager.NetworkCallback() { // from class: com.tencent.liteav.base.networkbinder.NetworkBinder.2
                    ...
                    public final void onAvailable(Network network) {
                        ...
                        if (network == null) {
                            Log.m46644e(NetworkBinder.TAG, "[BindSocket]:onAvailable, network is null", new Object[0]);
                            return;
                        }
                            ...
                            try {
                                if (NetworkBinder.this.mIsActivatingNetwork) {
                                    ...
                                    if (linkProperties != null && NetworkBinder.this.checkNativeValid()) {
                                        ...
                                        Log.m46645i(NetworkBinder.TAG, "[BindSocket]:start network succ, network name=".concat(String.valueOf(interfaceName)), new Object[0]);
                                    }
                            ...
                            try {
                                if (NetworkBinder.this.mIsNetworkAvailable && !NetworkBinder.this.mIsActivatingNetwork) {
                                    ...
                                    if (NetworkBinder.this.checkNativeValid()) {
                                        NetworkBinder.nativeOnNetworkAvailable(NetworkBinder.this.mNativeNetworkBinderJni, "", false);
                                        Log.m46645i(NetworkBinder.TAG, "[BindSocket]:network onLost", new Object[0]);
                                    }
                                    NetworkBinder.this.deactivateNetworkAdapter();
                                }
                            } finally {
    ...
    public NetworkBinder(long j10) {
        this.mNativeNetworkBinderJni = j10;
    }

    private static boolean checkSystemVersionValid() {
        if (LiteavSystemInfo.getSystemOSVersionInt() < 23) {
            if (isPrint) {
                LiteavLog.m46693e(TAG, "[BindSocket]:The android version is too low and does not support network binding");
                isPrint = false;
            }
            return false;
        }
    ...
    public void deactivateNetworkAdapter() {
        ...
        try {
            ...
            synchronized (this.mLocker) {
                try {
                    if (this.mNetworkCallback != null) {
                        Log.m46645i(TAG, "[BindSocket]:deactivat network adapter", new Object[0]);
            ...
            LiteavLog.m46693e(TAG, "[BindSocket]:deactivateNetworkAdapter in java encountered an exception");
        }
    }
